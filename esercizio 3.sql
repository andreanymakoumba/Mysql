...
from docenti, corsi
where docenti.id = corsi.docente_id
and titolo regexp '^Introduzione'
...
e ordina per cognome e nome
*/
select cognome, nome, email, genere
from studenti, corsi, iscrizioni
where studenti.id = iscrizioni.studente_id
and corsi.id = iscrizioni.corso_id and genere = "F"
order by cognome, nome;
...
from studenti, corsi, iscrizioni
where studenti.id = iscrizioni.studente_id
and corsi.id = iscrizioni.corso_id and titolo = "Java"
order by cognome, nome;
/* 9
...
from studenti, corsi, iscrizioni
where studenti.id = iscrizioni.studente_id
and corsi.id = iscrizioni.corso_id and titolo = "Basi di dati" and genere = "M"
order by cognome, nome;
...
from studenti, corsi, iscrizioni
where studenti.id = iscrizioni.studente_id
and corsi.id = iscrizioni.corso_id and iscrizioni.prezzo >= 200 
order by cognome, nome